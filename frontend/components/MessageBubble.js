  ...
  return (
    <div className={`group flex ${isMine ? "justify-end" : "justify-start"} mb-2`}>
      <div className={`relative max-w-[78%] sm:max-w-[65%]`}>
        {!isMine && sender?.name && (
          <p className="text-[11px] text-sky-600 font-medium mb-0.5 ml-1">
            {sender.name}
          </p>
        )}
        ...
        <div
          className={`rounded-2xl px-3.5 py-2 shadow-bubble ${
            isMine
              ...
              : "bg-white text-slate-800 border border-sky-50 rounded-bl-md"
          }`}
        >
          {deleted ? (
        ...
        {!deleted && (
          <button
            ...
            title="Delete for me"
            className={`absolute top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition text-xs px-2 py-1 rounded-lg bg-white border border-sky-100 text-slate-500 hover:text-red-500 hover:border-red-200 shadow-sm ${
